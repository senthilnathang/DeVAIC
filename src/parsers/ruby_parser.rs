...
impl RubyParser {
    pub fn new() -> Result<Self> {
        ...
        parser
            ...
            .map_err(|e| DevaicError::TreeSitter(format!("Error loading Ruby grammar: {}", e)))?;
        Ok(Self { parser })
    }
}
...
impl Parser for RubyParser {
    fn parse(&self, source_file: &SourceFile) -> Result<ParsedAst> {
        ...
        parser
            .set_language(tree_sitter_ruby::language())
            .map_err(|e| DevaicError::TreeSitter(format!("Error setting Ruby language: {}", e)))?;
        
        let tree = parser
            .parse(&source_file.content, None)
            .ok_or_else(|| DevaicError::Parse("Failed to parse Ruby code - syntax error or timeout".to_string()))?;
